...
export const Chat = ({ channelId }) => {
  ...
  const getMessageLog = React.useCallback(
    ({ channelId, callback }) => {
  ...
  return (
    <div className="chat">
      <ChatHeader
        info={exportInfoHeader(store.messageStore.list_message_log)}
        ownerId={store.messageStore.user_detail.id}
        header_info = {store.homeStore.header_info}
      />
      <ChatBody
